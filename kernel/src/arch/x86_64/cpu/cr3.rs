use crate::arch::{
    addr::{AllMemory, PAGE_MASK, Physical},
    frame::Frame,
};
...
pub fn read() -> Frame {
    ...
    Frame::new(Physical::<AllMemory>::new(cr3 & !PAGE_MASK))
}
...
/// the page table base pointer. This will cause the CPU to flush all
/// non-global TLB entries.
///
/// This is safe to use because flushing the TLB has no side effects except
/// for the performance impact of flushing the TLB. Code that relies on the
/// TLB containing stale entries should not be used in the first place as
