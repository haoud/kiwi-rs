...
bitflags! {
    ...
    pub struct Flags : u64 {
        ...
        /// cleared to disable them. This flag can be armed with the `sti`
        ...
        /// requires this flag to be cleared on function entry (used by
        /// the kernel).
        const DF = 1 << 10;

        /// Overflow flag. Set if a signed arithmetic operation generates a
        /// result too large to be represented in the destination operand,
        /// cleared otherwise.
        const OF = 1 << 11;

        /// I/O privilege bit 0. This flag is used in conjunction with the I/O
        /// privilege bit 1 to form a 2-bit field that specifies the minimum
        /// privilege level required to execute I/O instructions.
        const IOPL0 = 1 << 12;

        /// I/O privilege bit 1. This flag is used in conjunction with the I/O
        ...
        /// cleared to return to protected mode. This flag is not supported
        /// on `x86_64`.
        const VM = 1 << 17;
...
///
/// Flags that are used by the compiler for its own purposes (for example, the
/// carry flag or the direction flag) should not be modified by this function,
/// and doing so may be ignored by the compiler since the compiler is made
/// aware that this function modifies the RFLAGS register and is free to
/// restore the previous value of the flags after this function returns.
///
/// # Safety
