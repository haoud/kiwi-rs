...
impl<T: Copy> SeqLock<T> {
    ...
    pub fn write(&self, data: T) {
        ...
        // and the reader will wait for it to become even before
        // reading the data.
        self.seq.fetch_add(1, Ordering::Relaxed);

        // Make sure that the write of the data happens after incrementing
        // the sequence number. Ideally, we would use `Acquire` ordering,
        // but unfortunately the `Acquire` ordering is not available for
        // `store` operations.
        core::sync::atomic::fence(Ordering::Release);

        // Write the data using a volatile write, because the data
        // may be concurrently read by the reader.
        ...
        // Increment the sequence number to indicate that the data
        // is written. The sequence number will become even, and the
        // reader will be able to read the data.
        self.seq.fetch_add(1, Ordering::Relaxed);
    }

    /// Read data from the `SeqLock`. This function reads the data
    /// volatilely, and will use the sequence number to ensure that
    /// the read is consistent and was not modified during the read.
    /// If the data is being written to, this function will spin until
    /// the write is complete.
    pub fn read(&self) -> T {
        loop {
            // Load the sequence number. We use the `Acquire` ordering
            // to ensure that the read of the data happens after the
            // read of the sequence
            let seq = self.seq.load(Ordering::Acquire);

            // If the sequence number is odd, the data is being written
            // to. We need to wait for the writer to finish.
            if (seq & 1) == 1 {
                core::hint::spin_loop();
                continue;
            }

            // Read the data using a volatile read, because the data may
            ...
            // ordering, but unfortunately the `Release` ordering is not
            // available for `load` operations.
            core::sync::atomic::fence(Ordering::Acquire);

            // SAFETY: If the sequence number is still the same, the read
            // was consistent so the data was not modified during the read,
            // and we can safely assume it is initialized.
            unsafe {
